...
- **What the contract holds equal is the shared `extract_strings` MCP tool**,
...
- **An unrecognised format falls back, it does not fail.** That is true here
  and in the crate, and it is what lets either be pointed at a source tree.
  See `crate/SPEC.md`; contract tests on both surfaces enforce it.
