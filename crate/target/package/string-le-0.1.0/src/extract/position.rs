...
impl<'a> PositionIndex<'a> {
    ...
    /// to that character's start — neither can happen from a parser
    /// range or a substring search, but a silently wrong column would be
    /// worse than a defensive floor.
