...
//! precaution. A file this does not recognise falls through to
//! quoted-string extraction, so a `.ts` full of user-facing copy is read
//! by the walk rather than skipped by it — which is the whole audit.
//!
...
mod tests {
    ...
    /// Every text file, whatever its extension. A source file is where
    /// the user-facing copy lives, and it is not a format this parses.
    #[test]
    ...
    fn ignored_files_are_skipped() {
        ...
        tree.write("ignored.ts", "const a = 'x';");
        tree.write("kept.ts", "const b = 'y';");
