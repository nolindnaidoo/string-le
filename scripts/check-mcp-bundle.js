...
setTimeout(() => {
	...
	if (init?.serverInfo?.name !== 'string-le') {
	...
	if (call?.isError !== false) {
		fail('extract_strings reported an error', JSON.stringify(call));
	}
	const found = call?.structuredContent?.data?.strings ?? [];
	// Asserting the actual values, not just "it responded" — a server that
	// answers with an empty list would otherwise pass.
	for (const expected of ['widget', 'team']) {
		if (!found.includes(expected)) {
			fail(`extract_strings did not find ${expected}`, `got: ${found.join(', ')}`);
		}
	}
