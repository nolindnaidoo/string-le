...
exports.run = async function run() {
	...
	// Smoke the primary command end to end: JSON values.
	const jsonDoc = await vscode.workspace.openTextDocument({
		content: '{"greeting": "hello world", "nested": {"msg": "second value"}, "n": 7}',
		language: 'json',
	});
	await vscode.window.showTextDocument(jsonDoc);
	await vscode.commands.executeCommand('string-le.extractStrings');
	const jsonResult = vscode.workspace.textDocuments.find(
		(d) => d.languageId === 'plaintext' && d.getText().includes('hello world'),
	);
	assert.ok(jsonResult, 'no JSON results document produced');
	assert.deepStrictEqual(jsonResult.getText().split('\\n'), [
		'hello world',
		'second value',
	]);

	// And unquoted YAML scalars — the real-parser path.
	const yamlDoc = await vscode.workspace.openTextDocument({
		content: 'title: Plain unquoted value\\nquoted: "Quoted value"\\n',
		language: 'yaml',
	});
	await vscode.window.showTextDocument(yamlDoc);
	await vscode.commands.executeCommand('string-le.extractStrings');
	const yamlResult = vscode.workspace.textDocuments.find(
		(d) =>
			d.languageId === 'plaintext' &&
			d.getText().includes('Plain unquoted value'),
	);
	assert.ok(yamlResult, 'no YAML results document produced');
	assert.deepStrictEqual(yamlResult.getText().split('\\n'), [
		'Plain unquoted value',
		'Quoted value',
	]);
	console.log('VSIX E2E OK: JSON + YAML extraction verified');
