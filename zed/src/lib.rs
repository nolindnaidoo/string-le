//! The Zed side of String-LE.
//!
//! This crate holds no extraction logic. It resolves `string-le-mcp` from npm and
//! hands Zed a command that launches it over stdio — the same server the VSIX
//! bundles and the same one `npx string-le-mcp` runs. Keeping it a launcher is
...
const PACKAGE_NAME: &str = "string-le-mcp";
const SERVER_PATH: &str = "node_modules/string-le-mcp/server.js";

struct StringLEExtension;
...
impl zed::Extension for StringLEExtension {
    fn new() -> Self {
        Self
    }
...
zed::register_extension!(StringLEExtension);
